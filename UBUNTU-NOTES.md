...
The primary change introduced in this branch was to `versions.sh`, adding `jammy`, `noble`, `jammy-slim` and `noble-slim` variants. The other changes are generated by executing the following command, which uses Docker so it works on any platform.

```shell
docker run -it --rm -v $(pwd):/app ubuntu
apt-get update
apt-get install -y bsdmainutils curl gawk git jq wget
/app/update.sh
```
...
Several tags are built, the main one is `docker.io/cartesi/python:noble`.
